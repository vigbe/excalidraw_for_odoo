...
class TestExcalidrawChatterController(HttpCase):
    ...
    def test_save_upsert_no_duplicate(self):
        ...
        first = self._save(
            "exc_full", "exc_full",
            res_model="res.partner", res_id=host.id,
            name="Sketch", scene=SCENE_V1, png=PNG_A_B64,
        )
        ...
        second = self._save(
            "exc_full", "exc_full",
            res_model="res.partner", res_id=host.id,
            name="Sketch", scene=SCENE_V2, png=PNG_B_B64,
        )
    ...
    def test_save_last_write_wins(self):
        ...
        first = self._save(
            "exc_full", "exc_full",
            res_model="res.partner", res_id=host.id,
            name="Race", scene=SCENE_V1, png=PNG_A_B64,
        )
        second = self._save(
            "exc_full", "exc_full",
            res_model="res.partner", res_id=host.id,
            name="Race", scene=SCENE_V2, png=PNG_B_B64,
        )
        self.assertNotIn("error", first)
        self.assertNotIn("error", second)
        self.assertEqual(len(self._host_attachments(host)), 2)
    ...
    def test_save_png_twin_recreated(self):
        ...
        first = self._save(
            "exc_full", "exc_full",
            res_model="res.partner", res_id=host.id,
            name="Twin", scene=SCENE_V1, png=PNG_A_B64,
        )
        ...
        second = self._save(
            "exc_full", "exc_full",
            res_model="res.partner", res_id=host.id,
            name="Twin", scene=SCENE_V2, png=PNG_B_B64,
        )
        self.assertNotIn("error", second)
        self.assertEqual(second["scene_id"], first["scene_id"])
        attachments = self._host_attachments(host)
    ...
    def test_save_invalid_png_rejected(self):
        ...
        result = self._save(
            "exc_full", "exc_full",
            res_model="res.partner", res_id=host.id,
            name="BadPng", scene=SCENE_V1, png="@@not-base64@@",
        )
    ...
    def test_save_empty_elements_scene_only(self):
        ...
        result = self._save(
            "exc_full", "exc_full",
            res_model="res.partner", res_id=host.id,
            name="Blank", scene=EMPTY_ELEMENTS_SCENE, png=None,
        )
        self.assertNotIn("error", result)
        self.assertTrue(result.get("ok"))
    ...
    def test_save_empty_name_fallback(self):
        ...
        result = self._save(
            "exc_full", "exc_full",
            res_model="res.partner", res_id=host.id,
            name="", scene=SCENE_V1, png=PNG_A_B64,
        )
        self.assertNotIn("error", result)
        attachments = self._host_attachments(host)
        self.assertEqual(len(attachments), 2)
    ...
    def test_save_group_denied(self):
        ...
        result = self._save(
            "exc_outsider", "exc_outsider",
            res_model="res.partner", res_id=host.id,
            name="Nope", scene=SCENE_V1, png=None,
        )
        self.assertIn("error", result)
        self.assertEqual(len(self._host_attachments(host)), 0)
    ...
    def test_save_no_write_denied(self):
        ...
        result = self._save(
            "exc_plain", "exc_plain",
            res_model="project.task", res_id=self.deny_task.id,
            name="Denied", scene=SCENE_V1, png=None,
        )
        self.assertIn("error", result)
    ...
    def test_save_unknown_or_nonchatter_model_generic(self):
        ...
        unknown = self._save(
            ...
            name="X", scene=SCENE_V1, png=None,
        )
        nonchatter = self._save(
            "exc_full", "exc_full",
            res_model="res.company", res_id=1,
            name="X", scene=SCENE_V1, png=None,
        )
    ...
    def test_scene_fetch_by_id(self):
        ...
        saved = self._save(
            "exc_full", "exc_full",
            res_model="res.partner", res_id=host.id,
            name="Fetch", scene=SCENE_V1, png=None,
        )
    ...
    def test_scene_fetch_denied(self):
        ...
        saved = self._save(
            "exc_full", "exc_full",
            res_model="res.partner", res_id=host.id,
            name="Denied Fetch", scene=SCENE_V1, png=None,
        )
